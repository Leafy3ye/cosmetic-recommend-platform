...
本项目是一个基于SpringBoot + Vue3的前后端分离的化妆品电商推荐平台，集成了完整的电商功能和协同过滤推荐算法。系统支持用户注册登录、商品浏览、收藏、足迹、购物车、订单管理等基础电商功能，并预留了基于用户协同过滤（UserCF）和物品协同过滤（ItemCF）的个性化推荐算法接口。
...
| GET | /api/favorite/user/{userId} | 获取用户收藏商品 |
...
| GET | /api/behavior/history/{userId} | 获取用户浏览足迹商品 |
...
3. JWT Token有效期为24小时
4. Token过期会自动清理登录态，公开页面可继续访问，受保护页面会跳转登录
5. 图片上传功能需配置文件存储路径
6. 后端已关闭MyBatis行级SQL明细日志，避免控制台刷屏
...
本项目采用 `Graduate Project Reference License v1.0`（毕业设计参考授权协议）。  
项目开源仅用于版本管理、学习参考与技术交流，禁止商用及学术作弊/盗用。  
完整条款请查看仓库根目录 `LICENSE` 文件。
